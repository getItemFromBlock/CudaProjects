...
__device__ f32 Resources::Mesh::BoundsCheck(Compute::Ray r, Maths::Vec2 bounds) const
{
    return RayTracing::HitBox(r, transformedBox, bounds);
}

__device__ HitRecord Mesh::Intersect(Ray r, Vec2 bounds, bool inverted) const
{
    HitRecord result;
    if (RayTracing::HitBox(r, transformedBox, bounds) == INFINITY) return result;
    Vec3 verts[3];
    for (u32 i = 0; i < indiceCount; i += 3)
    {
        ...
        HitRecord hit = RayTracing::HitTriangle(r, verts, bounds);
